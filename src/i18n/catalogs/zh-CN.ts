const zhCN = {
  ...
  addDownloads: {
    ...
    destinationCheckFailed: '无法检查目标位置。请重试或选择其他位置。',
    backendRejectedStart: '后端拒绝开始下载。',
    playlistNoEntries: '播放列表不包含可下载的条目',
    invalidMediaMetadata: '媒体元数据无效或未找到格式',
    cookieFileInvalid: '所选 Cookie 文件不可用或不是普通文件。请在设置 → 媒体提取器中重新选择。',
    youtubeBotBlocked: 'YouTube 将此请求识别为自动请求并阻止了它。请选择已登录的浏览器来源或 Netscape Cookie 文件，然后刷新元数据。',
    youtubeAuthenticationRequired: 'YouTube 要求为此媒体进行身份验证。请登录浏览器账户、导出 Netscape Cookie，或选择正确的浏览器来源，然后刷新元数据。',
    youtubePoTokenRequired: 'YouTube 要求此客户端提供有效的 PO token。请尝试已登录的浏览器来源或 Netscape Cookie 文件，然后刷新元数据。',
    ...
    treatAsMedia: '按媒体处理',
    treatAsFile: '按文件处理',
    mediaModeUnavailable: '媒体模式需要不含嵌入凭据的 HTTP(S) URL。',
    mediaFormat: '媒体格式',
  ...
  settings: {
    ...
    lookAndFeel: {
      ...
      startAtLogin: '登录系统时自动启动',
      startAtLoginDescription: '登录系统时在后台启动 Firelink。窗口会保持隐藏，直到你从托盘打开它。',
      startAtLoginUnsupported: '此平台不支持自动启动。',
      startAtLoginRequiresApproval: '需要在系统登录项设置中批准。',
      openLoginItemsSettings: '打开登录项设置',
      startAtLoginCheckFailed: '无法检查自动启动状态：{{detail}}',
      startAtLoginUpdateFailed: '无法更新自动启动设置：{{detail}}',
      startAtLoginSettingsFailed: '无法打开登录项设置：{{detail}}',
      trayRequiredForStartAtLogin: '启用自动启动后，托盘图标会保持可用。',
    },
    network: {
      proxy: '代理',
      mode: '模式',
    ...
    engine: {
      ...
      cookieDescription: '公开媒体通常无需 cookies。如果网站要求登录，请选择已登录的浏览器；yt-dlp 会直接读取该浏览器的 cookies，因此无需手动导出。Firelink 不会保存浏览器 cookies 的内容。',
      cookieFile: 'Netscape Cookie 文件：',
      cookieFileDescription: '通常留空即可。选择了浏览器后，只有在无法读取浏览器 cookies 时 Firelink 才会尝试此 Netscape 格式文件。未选择浏览器时，此文件会作为 cookies 来源。Firelink 不会保存文件内容。',
      chooseCookieFile: '选择 Cookie 文件',
      clearCookieFile: '清除 Cookie 文件',
      none: '无',
    },
    integrations: {
      title: '连接浏览器扩展',
