const ru = {
  ...
  addDownloads: {
    ...
    destinationCheckFailed: 'Не удалось проверить место назначения. Повторите попытку или выберите другое место.',
    ...
    cookieFileInvalid: 'Выбранный файл cookie недоступен или не является обычным файлом. Выберите его снова в настройках → Экстракторы медиа.',
    youtubeBotBlocked: 'YouTube заблокировал запрос как автоматический. Выберите браузерный источник с выполненным входом или файл cookie Netscape и обновите метаданные.',
    youtubeAuthenticationRequired: 'Для этого медиа YouTube требует входа. Войдите в браузер, экспортируйте cookie Netscape или выберите правильный источник браузера и обновите метаданные.',
    youtubePoTokenRequired: 'Для этого клиента YouTube требует действительный PO-токен. Попробуйте браузерный источник с выполненным входом или файл cookie Netscape и обновите метаданные.',
    ...
    ready: 'Готово',
    mediaHandling: 'Обработка медиа',
    mediaModeAuto: 'Определять автоматически',
    treatAsMedia: 'Считать медиа',
    treatAsFile: 'Считать файлом',
    mediaModeUnavailable: 'Для медиарежима требуется URL HTTP(S) без встроенных учетных данных.',
  ...
  settings: {
    ...
    lookAndFeel: {
      ...
      startAtLogin: 'Запускать автоматически при входе в систему',
      startAtLoginDescription: 'Запускает Firelink в фоновом режиме при входе в систему. Окно остаётся скрытым, пока вы не откроете его из трея.',
      startAtLoginUnsupported: 'Автоматический запуск недоступен на этой платформе.',
      startAtLoginRequiresApproval: 'Требуется подтверждение в настройках элементов автозапуска системы.',
      openLoginItemsSettings: 'Открыть настройки автозапуска',
      startAtLoginCheckFailed: 'Не удалось проверить автоматический запуск: {{detail}}',
      startAtLoginUpdateFailed: 'Не удалось обновить автоматический запуск: {{detail}}',
      startAtLoginSettingsFailed: 'Не удалось открыть настройки автозапуска: {{detail}}',
      trayRequiredForStartAtLogin: 'Значок в трее остаётся доступным, пока включён автоматический запуск.',
    },
    network: {
      proxy: 'Прокси',
      mode: 'Режим',
    ...
    engine: {
      ...
      cookieDescription: 'Для общедоступных медиа обычно не нужны файлы cookie. Если сайт требует входа, выберите браузер, в котором выполнен вход; yt-dlp читает cookie напрямую из него, поэтому экспортировать их вручную не нужно. Firelink не сохраняет содержимое cookie браузера.',
      cookieFile: 'Файл cookie Netscape:',
      cookieFileDescription: 'Обычно оставляйте это поле пустым. Если выбран браузер, Firelink использует файл Netscape, только когда не удаётся прочитать cookie из браузера. Если браузер не выбран, файл становится источником cookie. Firelink не сохраняет его содержимое.',
      chooseCookieFile: 'Выбрать файл cookie',
      clearCookieFile: 'Очистить файл cookie',
