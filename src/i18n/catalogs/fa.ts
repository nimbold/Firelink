const fa = {
  ...
  downloads: {
    removal: { removing: "در حال حذف…", error: "حذف ناموفق بود", retry: "تلاش دوباره برای حذف", pending: "حذف دانلود در انتظار انجام است.", failed: "برنامه‌هایی را که از فایل‌ها استفاده می‌کنند ببندید، دسترسی به درایو و مجوزها را بررسی کنید و دوباره حذف کنید." },
    actions: {
  ...
  properties: {
    ...
    torrentPeerOptionsSavedHint: 'برای هر تورنت ذخیره می‌شود. این مقدار سقف اتصال است؛ ۰ یعنی نامحدود. مقدار خالی از پیش‌فرض آریا۲ یعنی حداکثر ۵۵ همتا استفاده می‌کند، بنابراین دانلودهای فعال معمولاً حدود ۴۴ همتای متصل نشان می‌دهند.',
    torrentPeerSpeedLimitHint: 'این مقدار محرکی بر اساس سرعت کلی دانلود است، نه سقف پهنای‌باند. آریا۲ وقتی سرعت تورنت کمتر از این مقدار باشد، موقتاً همتاهای بیشتری جست‌وجو می‌کند؛ این مقدار سرعت اینترنت شما را تشخیص نمی‌دهد یا هدف قرار نمی‌دهد. مقدار خالی از پیش‌فرض 50K آریا۲ استفاده می‌کند. مقادیر سرعت بر حسب بایت‌برثانیه هستند، مثل 50K یا 35M.',
  ...
  addDownloads: {
    ...
    destinationCheckFailed: 'بررسی مقصد ممکن نشد. دوباره تلاش کنید یا مقصد دیگری انتخاب کنید.',
    ...
    cookieFileInvalid: 'فایل کوکی انتخاب‌شده در دسترس نیست یا فایل معمولی نیست. آن را دوباره از تنظیمات → استخراج‌کننده‌های رسانه انتخاب کنید.',
    youtubeBotBlocked: 'یوتیوب این درخواست را خودکار تشخیص داد. یک منبع مرورگر واردشده یا فایل کوکی Netscape انتخاب کنید و فراداده را تازه کنید.',
    youtubeAuthenticationRequired: 'یوتیوب برای این رسانه به احراز هویت نیاز دارد. وارد حساب مرورگر شوید، کوکی Netscape صادر کنید یا منبع درست مرورگر را انتخاب کنید و فراداده را تازه کنید.',
    youtubePoTokenRequired: 'یوتیوب برای این کلاینت به PO token معتبر نیاز دارد. یک منبع مرورگر واردشده یا فایل کوکی Netscape را امتحان کنید و فراداده را تازه کنید.',
    ...
    torrentMaxPeers: 'حداکثر همتاهای تورنت',
    torrentPeerSpeedLimit: 'آستانه سرعت همتا',
    torrentPeerOptionsHint: 'برای استفاده از پیش‌فرض‌های آریا۲ خالی بگذارید (حداکثر ۵۵ همتا، معمولاً حدود ۴۴ همتای متصل هنگام دانلود، و آستانهٔ 50K). آستانه بر اساس سرعت کلی دانلود عمل می‌کند، نه سقف پهنای‌باند، و با سرعت اینترنت شما سازگار نمی‌شود. صفر همتا یعنی نامحدود. مقادیر سرعت بر حسب بایت‌برثانیه هستند، مثل 50K یا 35M.',
    torrentMaxPeersInvalid: 'حداکثر همتاهای تورنت باید عددی صحیح بین ۰ و ۱۰۰۰ باشد',
    torrentPeerSpeedLimitInvalid: 'آستانه سرعت همتای تورنت باید بیشتر از صفر باشد',
    torrentStopTimeout: 'توقف تورنتِ بدون سرعت پس از',
    ...
    invalid: 'نامعتبر',
    ready: 'آماده',
    mediaHandling: 'نحوهٔ مدیریت رسانه',
    mediaModeAuto: 'تشخیص خودکار',
    treatAsMedia: 'به‌عنوان رسانه',
    treatAsFile: 'به‌عنوان فایل',
    mediaModeUnavailable: 'حالت رسانه به یک نشانی HTTP(S) بدون اعتبارنامهٔ جاسازی‌شده نیاز دارد.',
  ...
  settings: {
    ...
    lookAndFeel: {
      ...
      startAtLogin: 'شروع خودکار هنگام ورود به سیستم',
      startAtLoginDescription: 'با ورود شما، فایرلینک را در پس‌زمینه اجرا می‌کند. پنجره تا زمانی که آن را از سینی باز نکنید پنهان می‌ماند.',
      startAtLoginUnsupported: 'شروع خودکار در این سیستم‌عامل در دسترس نیست.',
      startAtLoginRequiresApproval: 'تأیید در تنظیمات موارد ورود سیستم لازم است.',
      openLoginItemsSettings: 'باز کردن تنظیمات موارد ورود',
      startAtLoginCheckFailed: 'بررسی شروع خودکار ممکن نیست: {{detail}}',
      startAtLoginUpdateFailed: 'به‌روزرسانی شروع خودکار ممکن نیست: {{detail}}',
      startAtLoginSettingsFailed: 'باز کردن تنظیمات موارد ورود ممکن نیست: {{detail}}',
      trayRequiredForStartAtLogin: 'تا زمانی که شروع خودکار فعال است، آیکون سینی در دسترس می‌ماند.',
    },
    network: {
      proxy: 'پروکسی',
      mode: 'حالت',
    ...
    engine: {
      ...
      cookieDescription: 'رسانه‌های عمومی معمولاً بدون کوکی کار می‌کنند. اگر سایتی ورود به حساب را لازم می‌داند، مرورگری را انتخاب کنید که در آن وارد حساب شده‌اید؛ yt-dlp کوکی‌ها را مستقیماً از همان مرورگر می‌خواند و نیازی به خروجی‌گرفتن دستی نیست. Firelink محتوای کوکی‌های مرورگر را ذخیره نمی‌کند.',
      cookieFile: 'فایل کوکی Netscape:',
      cookieFileDescription: 'معمولاً این قسمت را خالی بگذارید. اگر مرورگری انتخاب شده باشد، Firelink فقط وقتی نتواند کوکی‌های آن را بخواند از فایل Netscape استفاده می‌کند. اگر مرورگری انتخاب نشده باشد، این فایل منبع کوکی خواهد بود. Firelink محتوای فایل را ذخیره نمی‌کند.',
      chooseCookieFile: 'انتخاب فایل کوکی',
      clearCookieFile: 'پاک کردن فایل کوکی',
      none: 'هیچ‌کدام',
    },
    integrations: {
